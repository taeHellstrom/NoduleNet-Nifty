...
def preprocess_nifty(params):
    '''
    Alternative preprocessing script to use for nifty files
    :param  params
            [case_num, lung_mask_dir, nod_mask_dir, img_dir, save_dir, do_resample]
            :param case_num: name of case without ending
            :param lung_mask_dir: Path to folder containing lung masks
            :param nod_mask_dir: Path to folder containing nodule masks
            :param save_dir: Path to folder where data will be saved
            :param do_resample: Boolean operator. If True data will be resampled to common spacing
    '''
    
    case_num, lung_mask_dir, nod_mask_dir, img_dir, save_dir, do_resample, time_start = params
    time_temp = time.time()
    # if  'LUNG SCREEN-34461' in case_num or (time_temp-time_start)>10:
    # idx = pid.find('.')
    # case_num = pid[:idx]
    print('Processing case: ', case_num)
    pid = case_num + '.nii.gz'
    nodule_names = os.listdir(nod_mask_dir)
    i=0
    for nod_name in nodule_names:
        if case_num in nod_name:
            i+=1
            if i == 1:
                pid_nod = nod_name
            else:
                raise Exception(f"Several nodule segmentation files for same case:\n{pid_nod}\n{nod_name}")
    img, origin, spacing = load_itk_image(os.path.join(img_dir, pid))
    nod_mask, _, _ = load_itk_image(os.path.join(nod_mask_dir, pid_nod))
    # if (nod_mask > 1).any:
    #     nod_mask = np.where(nod_mask> 0, 1, 0) 
    i=0
    if os.path.exists(lung_mask_dir):  
        
        for lung_name in os.listdir(lung_mask_dir):
            if case_num in lung_name:
                i+=1
                if i == 1:
                    pid_lung = lung_name
                else:
                    raise Exception(f"Several lung segmentation files for same case:\n{pid_lung}\n{lung_name}")

        lung_mask, _, _ = load_itk_image(os.path.join(lung_mask_dir, pid_lung))

    else:
        raise Exception(f"No lung masks for case {case_num}")
    

    binary_mask1, binary_mask2 = lung_mask == 1, lung_mask == 2     # pixel values corresponding to left and right lung
    binary_mask = binary_mask1 + binary_mask2
    ...
    if do_resample:
        ...
        for i in range(int(nod_mask.max())):
            ...
            print('case: ', case_num)
            seg_nod_mask[mask > 0.5] = i + 1
            # print('mask seg min max', seg_nod_mask.min(), seg_nod_mask.max(), f'i{i}')
    lung_box = get_lung_box(binary_mask, seg_img.shape)
    ...
    seg_img = seg_img[z_min:z_max, y_min:y_max, x_min:x_max]
    seg_nod_mask = seg_nod_mask[z_min:z_max, y_min:y_max, x_min:x_max]
    # print('mask seg min max', seg_nod_mask.min(), seg_nod_mask.max(), f'idx {[z_min,z_max, y_min,y_max, x_min,x_max]}')
    np.save(os.path.join(save_dir, '%s_origin.npy' % (case_num)), origin)
    np.save(os.path.join(save_dir, '%s_spacing.npy' % (case_num)), resampled_spacing)
    np.save(os.path.join(save_dir, '%s_ebox_origin.npy' % (case_num)), np.array((z_min, y_min, x_min)))
    nrrd.write(os.path.join(save_dir, '%s_clean.nrrd' % (case_num)), seg_img)
    nrrd.write(os.path.join(save_dir, '%s_mask.nrrd' % (case_num)), seg_nod_mask)

    print('number of nodules before: %s, after preprocessing: %s' % (nod_mask.max(), seg_nod_mask.max()))
    print('Finished %s' % (case_num))
    time_now = time.time()
    print(f'Time passed: {round((time_now-time_start)/60)} min and {round((time_now-time_start)%60)} sec')
    print()

# '/media/terese/New Volume1/Lung_cancer_project/Nifty_data_LIDC-IDRI/temp_numpy_dir/nod_segs/0002_image.nii.gz'

def generate_label(params):
    pid, lung_mask_dir, nod_mask_dir, img_dir, save_dir, do_resample, time_start = params
...
def main():
    ...
    nod_mask_dir = os.path.join(config['mask_save_dir'])
    ...
    for pid in os.listdir(img_dir):
        idx = pid.find('.')
        case_num = pid[:idx]
        params_lists.append([case_num, lung_mask_dir, nod_mask_dir, img_dir, save_dir, do_resample, time_start])
    
    pool = Pool(processes=1)
    pool.map(preprocess_nifty, params_lists)
    print('\n\nDone\n\n')
    pool.close()
    pool.join()
