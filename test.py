...
from net.nodule_net import NoduleNet    #Warning: C++ module import failed! This should only happen in deployment
...
# The following line has this error: /home/terese/anaconda3/envs/env_NoduleNet/lib/python3.7/site-packages/IPython/html.py:14: ShimWarning: The `IPython.html` package has been deprecated since IPython 4.0. You should import from `notebook` instead. `IPython.html.widgets` has moved to `ipywidgets`.
...
def main():
    ...
    if args.mode == 'eval':
        ...
        print('Dirs', data_dir, test_set_name)
...
def eval(net, dataset, save_dir=None):
    ...
    for i, (input, truth_bboxes, truth_labels, truth_masks, mask, image) in enumerate(dataset):
        try:
            ...
            # print('GT mask', type(mask), mask.min(), mask.max())
            gt_mask = mask.astype(np.uint8)
            # print('GT mask2', type(gt_mask), gt_mask.min(), gt_mask.max())
    ...
    noduleCADEvaluation('evaluationScript/annotations/LIDC/bbox_data.csv',
